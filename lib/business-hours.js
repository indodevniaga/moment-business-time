...
moment.fn.workingDiff = function workingDiff(comparator, unit, detail) {
    ...
    while(from.format('L') !== to.format('L')) {
		if (unit === 'day') {
			...
			if (from.isSameOrBefore(openingTimes(from)[3])){
				if(from.isAfter(openingTimes(from)[2])){
					diff += openingTimes(from)[3].diff(from,unit,true);
				}
				else{
					diff += openingTimes(from)[3].diff(openingTimes(from)[2],unit,true);
				}
			}
    ...
    if (unit !== 'day') {
        ...
        if(from.isSameOrBefore(openingTimes(from)[3])){
            if(from.isSameOrBefore(openingTimes(from)[2]) && to.isSameOrAfter(openingTimes(from)[3])){
				diff += openingTimes(from)[3].diff(openingTimes(from)[2],unit,true);
			}
			else{
				if(from.isSameOrAfter(openingTimes(from)[2]) && to.isSameOrBefore(openingTimes(from)[3])){
					diff += to.diff(from, unit, true);
				}
				else{
					if(from.isSameOrAfter(openingTimes(from)[2]) && to.isAfter(openingTimes(from)[3])){
						diff +=  openingTimes(from)[3].diff(from,unit,true);
                    }
                    else{
                        if(to.isSameOrAfter(openingTimes(from)[2])&& to.isSameOrBefore(openingTimes(from)[3])){
                            diff += to.diff(openingTimes(from)[2], unit, true);
                        }
                    }
				}
			}
